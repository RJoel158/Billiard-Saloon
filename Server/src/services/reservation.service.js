...
async function getReservationById(id) {
  ...
  if (!r) throw new ApiError(404, 'RESERVATION_NOT_FOUND', 'Reserva no encontrada');
  return r;
}
...
async function createReservation(data) {
  ...
  if (!data.user_id || !data.table_id || !data.start_time || !data.end_time) {
    throw new ApiError(400, 'MISSING_FIELDS', 'Faltan campos requeridos para la reserva');
  }
  ...
  if (durationHours < 1) {
    throw new ApiError(400, 'INVALID_DURATION', 'La duración mínima de reserva es 1 hora');
  }
  ...
  const conflicts = await repository.findByTableAndDateRange(data.table_id, startTime, endTime);
  if (conflicts.length > 0) {
    throw new ApiError(409, 'TIME_CONFLICT', 'Ya existe una reserva o sesión activa en ese horario');
  }

  // Check for active session on the table
  const activeSession = await sessionRepo.findActiveByTable(data.table_id);
  if (activeSession) {
    const sessionEnd = activeSession.end_time ? new Date(activeSession.end_time) : null;
    if (!sessionEnd || sessionEnd > startTime) {
      throw new ApiError(409, 'TABLE_OCCUPIED', 'La mesa está ocupada actualmente');
    }
  }
...
async function approveReservation(id, admin_user_id) {
  ...
  if (reservation.status !== 1) {
    throw new ApiError(400, 'INVALID_STATUS', 'Solo se pueden aprobar reservas pendientes');
  }
...
async function rejectReservation(id, admin_user_id, reason) {
  ...
  if (reservation.status !== 1) {
    throw new ApiError(400, 'INVALID_STATUS', 'Solo se pueden rechazar reservas pendientes');
  }
...
async function updateReservation(id, data) {
  ...
  const payload = {
    ...
    reservation_date: data.reservation_date !== undefined ? data.reservation_date : existing.reservation_date,
