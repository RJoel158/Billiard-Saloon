...
async function getActive(req, res, next) {
  try {
    const items = await service.getActiveSessions();
    res.json(items);
  } catch (err) {
    next(err);
  }
...
async function close(req, res, next) {
  try {
    const result = await service.closeSession(req.params.id, req.body.end_time);
    res.json(result);
  } catch (err) {
    next(err);
  }
...
module.exports = { getAll, getActive, getById, create, update, close, deleteSession };
